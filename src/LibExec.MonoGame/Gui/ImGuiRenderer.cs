...
public sealed class ImGuiRenderer
{
    ...
    private readonly Assembly _assembly = Assembly.GetEntryAssembly() ?? throw new NullReferenceException();
    ...
    /// </summary>
    internal void BeforeLayout(GameTime gameTime)
    {
    ...
    public ImFontPtr AddFont(string path, int size, bool rebuildFontAtlas = false)
    {
        ...
        var fontDataIntPtr = Marshal.AllocHGlobal(fontData.Length);
        Marshal.Copy(fontData, 0, fontDataIntPtr, fontData.Length);
        return fontAtlas.AddFontFromMemoryTTF(fontDataIntPtr, fontData.Length, size);
    }
